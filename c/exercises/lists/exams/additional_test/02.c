...
int main() {
	setlocale(LC_ALL, "");
	int matrizA[5][3], matrizCopia[5][3], lin = 0, col = 0, numRepetidos = 0, i = 0;
	...
	// Faz uma cópia dos valores digitados
	for (lin = 0; lin < 5; lin++) {
		for(col = 0; col < 3; col++) {
	...
	// Verifico antes quantas vezes os números repetiram 
	for (lin = 0; lin < 5; lin++) {
		for(col = 0; col < 3; col++) {
	...
	// c) Escrever cada elemento repetido com uma mensagem dizendo o elemento que aparece X vezes em A (2,5) [X]
	printf("\nNúmero  |  Quantia de vezes repetido");
